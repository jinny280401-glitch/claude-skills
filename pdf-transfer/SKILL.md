...
name: pdf-transfer
version: 1.1.0
description: 把结构化文本/数据转成 A4 PDF 的工作流(详细报告场景)。核心约束：技术不要留白 — 每页都要塞满有意义的内容。集成 weasyprint（主力）+ headless Chrome（备选）双引擎。触发条件：用户提到"导出 PDF"、"详细报告"、"做成 PDF 报告"、"留白太多"、"压紧一点"等。**注意：简讯/盘前快报不触发本 skill，直接用 chat markdown 表格输出(成本低 7-13 倍)**。跨平台支持 macOS / Linux / 统信 UOS / Windows。
---
...
- 详细报告型 PDF（集合竞价详细报告、研究报告、内参）— **本 skill 主推**
- 表格密集型 PDF（数据清单、对比表、矩阵）
- Word-like 文档（合同、纪要、SOP、模板）
- 需要"留白少、专业感"的任何 A4 输出

**不推荐走本 skill**：
- 简讯/盘前快报/Morning Brief → 直接 chat 输出 markdown 表格（成本低 7-13 倍,时效 2-4x）
...
# auction-report.html        ← ✅ 集合竞价详细报告(3-4 页,推荐)
# auction-morning-brief.html ← ⚠ deprecated — 简讯场景改用 chat markdown 表格
```
...
```bash
# 详细报告(PDF)
cp templates/auction-report.html my-report.html
# 或通用报告
